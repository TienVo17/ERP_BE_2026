...
public final class MasterDataModels {
    ...
    public record RawMaterialRequest(
            String category, @NotBlank String code, @NotBlank String name, String specification,
            String size, String color, @NotNull UUID uomId,
            @Pattern(regexp = "^\\d{1,12}(\\.\\d{1,6})?$") String referencePrice,
            @NotBlank @Pattern(regexp = "^[A-Z]{3}$") String currencyCode, UUID supplierId,
            @Pattern(regexp = "^\\d{1,14}(\\.\\d{1,4})?$") String safetyStockQty, String remark) {
    }

    public record RawMaterialResponse(
            UUID id, long version, String status, String category, String code, String name,
            String specification, String size, String color, UUID uomId, String uomCode,
            String referencePrice, String currencyCode, UUID supplierId, String supplierName,
            String safetyStockQty, String remark, Instant createdAt, Instant updatedAt) {
    }

    public record FinishedGoodRequest(
            @NotBlank @Pattern(regexp = "^(PRINT|WOVEN)$") String productKind, @NotBlank String styleNo,
            @NotBlank String name, String size, String color, @NotNull UUID uomId,
            @Pattern(regexp = "^\\d{1,12}(\\.\\d{1,6})?$") String referencePrice,
            @Pattern(regexp = "^[A-Z]{3}$") String currencyCode) {
    }

    public record FinishedGoodResponse(
            UUID id, long version, String status, String productKind, String styleNo, String name,
            String size, String color, UUID uomId, String uomCode, String referencePrice,
